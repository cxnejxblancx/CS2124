/*
  Author: Malani Snowde
  File: rec11.cpp
  Focus: Become acquinted with the basics of linked lists
*/
...
/* OPTIONAL */
const Node* sharedListConstSpace(const Node* listA, const Node* listB);
...
Node* reverse(const Node* hp) {
    if (hp == nullptr) { return new Node{}; }
    Node* result = nullptr;
    while(hp != nullptr) {
        listAddHead(result, hp->data);
        hp = hp->next;
    }
    return result;
} // reverse
...
const Node* isSublist(const Node* pattern, const Node* target) { 
    ...
    const Node* position = nullptr;
    while(target != nullptr) {
        const Node* ptrTarget = target;
        const Node* ptrPattern = pattern;
        while (ptrTarget != nullptr && ptrPattern != nullptr && ptrTarget->data == ptrPattern->data) {
                ptrPattern = ptrPattern->next;
                ptrTarget = ptrTarget->next;
        }
        if (ptrPattern == nullptr) { return target; }
        target = target->next;
    }
...
const Node* sharedListBruteForce(const Node* listA, const Node* listB) {
    const Node* ptrA = listA;
    while (ptrA != nullptr) {
        const Node* ptrB = listB;
        while (ptrB != nullptr) {
    ...
    return nullptr;
} // sharedListBruteForce

const Node* sharedListUsingSet(const Node* listA, const Node* listB) {
    unordered_set<const Node*> sharedList;
    while (listA != nullptr) {
        sharedList.insert(listA);
        listA = listA->next;
    }
    while (listB != nullptr) {
        if (sharedList.find(listB) != sharedList.end()) { return listB; }
        listB = listB->next;
    }
    return nullptr;
} // sharedListUsingSet
...
const Node* sharedListConstSpace(const Node* listA, const Node* listB) {
    ...
    const Node* ptrA = listA;
    const Node* ptrB = listB;
    const Node* shorterList;
    const Node* longerList;
    ...
    const Node* ptrLong = longerList;
    while (ptrLong != nullptr) {
        if (iters > 0) {
            ptrLong = ptrLong->next;
            --iters;
        }
        else {
            const Node* ptrShort = shorterList;
            while (ptrShort != nullptr) {
                if (ptrLong == ptrShort) { return ptrShort; }
